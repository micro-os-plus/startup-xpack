...
// defined, the code is capable of initialising multiple regions.
//
// Note: External memory with variable size (size known after reading the
// chip type) cannot be initialised via these linker script static tables
...
// the location of various points in the "Memory regions initialisation
...
#endif /* defined(
           MICRO_OS_PLUS_STARTUP_INITIALISE_MULTIPLE_RAM_SECTIONS_ENABLED) */
...
// for this file only.
//
// Note: replacing the comparisons/loops below with memcpy()/memset()
// would not avoid this UB; computing a byte count for them still
// requires pointer subtraction (e.g. region_end - region_begin),
// which has the same "same array/object" requirement as the
// comparisons used here. It would only relocate the UB, not remove
// it, while adding a dependency on library code being safe to call
// this early in the boot sequence.
...
extern "C"
{
  ...
  micro_os_plus_startup_initialise_data (std::uintptr_t* from,
                                         ...
                                         std::uintptr_t* region_end) noexcept;
...
/**
 * @brief Copy a DATA region from flash to RAM.
 * @param [in] from Pointer to the first word of the initialisation
 *  values, in flash.
 * @param [in] region_begin Pointer to the first word of the RAM
 *  region to initialise.
 * @param [in] region_end Pointer to one word past the last word of the
 *  RAM region to initialise.
 * @par Returns
 *  Nothing.
 *
 * @details
 * Copies words one at a time from `from` to `region_begin`, advancing
 * both pointers, until `region_begin` reaches `region_end`. All three
 * pointers are assumed to be word aligned; the caller is responsible
 * for this, since misaligned accesses are not checked here.
 */
[[gnu::always_inline]]
inline void
micro_os_plus_startup_initialise_data (std::uintptr_t* from,
 ...
 *
 * @details
 * Writes zero to each word from `region_begin` up to (but excluding)
 * `region_end`. The region is assumed to be word aligned; the caller
 * is responsible for this, since misaligned accesses are not checked
 * here.
 */
[[gnu::always_inline]]
inline void
                                      ...
                                      std::uintptr_t* region_end) noexcept
{
  ...
  while (p < region_end)
    ...
    && defined(MICRO_OS_PLUS_STARTUP_GUARD_CHECKS_ENABLED)
...
/**
 * @brief The portable startup entry point.
 * @par Parameters
 *  None.
 * @par Returns
...
_start (void)
{
  ...
  // to accommodate for the initialisations duration.
